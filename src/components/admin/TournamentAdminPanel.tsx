...
export function TournamentAdminPanel() {
  ...
  const [linkableMatches, setLinkableMatches] = useState<
    Array<{ id: string; name: string; home_score: number | null; away_score: number | null; status: string }>
  >([]);
  const [roster, setRoster] = useState<
    Array<{ id: string; name: string; logo_url: string | null; kind: "player" | "team" }>
  >([]);
  ...
  async function loadTournaments() {
    ...
    const { data: lm } = await (supabase as any)
      .from("matches")
      .select("id,name,home_score,away_score,status")
      .eq("is_archived", false)
      .eq("is_virtual", false)
  ...
  async function generateBracketFromPreview() {
    ...
    for (let r = totalRounds; r >= 1; r--) {
      ...
      const { data, error } = await (supabase as any)
        ...
        .insert(rows)
        .select("id,slot,next_match_id,next_slot");
      if (error) {
        toast.error(error.message);
        return;
      }
  ...
  async function linkFutures(matchId: string) {
    ...
    await (supabase as any)
      .from("tournaments")
      .update({ futures_match_id: matchId || null })
      .eq("id", sel.id);
    loadTournaments();
  ...
  async function deleteTournament() {
    ...
    const ok = await confirm({
      ...
      description: "The whole bracket, participants and results will be permanently removed.",
  ...
  function openResult(m: TMatch) {
    if (!m.participant_a_id && !m.participant_b_id) {
      toast.info("Both slots are empty — winners from the previous round will appear here.");
      return;
    }
    setResultMatch(m);
  ...
  async function linkLiveMatch(matchId: string) {
    ...
    if (matchId) {
      ...
      if (lm) {
        await (supabase as any)
          .from("tournament_matches")
          .update({ score_a: lm.home_score ?? 0, score_b: lm.away_score ?? 0 })
          .eq("id", resultMatch.id);
        ...
        <Card className="glass-strong p-4 space-y-3">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">
              Tournament name (words, e.g. "E-Football Competition Bet")
            </Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Sunday Night Shootout" />
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Tagline (short slogan)</Label>
            <Input
              value={tagline}
              onChange={(e) => setTagline(e.target.value)}
              placeholder="ONE LEAGUE. NO MERCY. RESPECT THE GAME."
            />
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Event date (calendar)</Label>
            <Input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} />
          </div>
          <Button className="btn-luxury w-full" onClick={createTournament}>
            ...
            <div className="pt-2 space-y-1">
                ...
                {selectedTournaments.size > 0 && (
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      if (
                        !window.confirm(
                          `Delete ${n} tournament${n === 1 ? "" : "s"}? Their brackets, participants and results will be permanently removed.`,
                        )
                      )
                        return;
              ...
              <div className="flex flex-col gap-1 max-h-40 overflow-y-auto">
                {tournaments.map((t) => (
                  <div
                    key={t.id}
                    className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm border ${selId === t.id ? "border-primary bg-primary/10" : "border-border"}`}
                  >
                    <input
                      type="checkbox"
                      checked={selectedTournaments.has(t.id)}
                      onChange={() =>
                        setSelectedTournaments((s) => {
                          const n = new Set(s);
          ...
          <Card className="glass-strong p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="font-bold flex items-center gap-2">
                ...
                Participants — {sel.name}
              </div>
              <Button size="sm" variant="destructive" onClick={deleteTournament}>
                <Trash2 className="h-3 w-3 mr-1" />
                Delete
              </Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2 items-end">
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">
                  Participant — pick a seeded shooter/gang or type a name
                </Label>
                <div className="flex gap-2">
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      if (r) {
                        setPName(r.name);
                        if (r.logo_url) setPLogo(r.logo_url);
                      }
                    }}
                  >
                    ...
                    <SelectContent className="max-h-72">
                      {roster.length === 0 && (
                        <div className="px-2 py-3 text-xs text-muted-foreground">No seeded players/teams</div>
                      )}
                      {roster.map((r) => (
                        <SelectItem key={r.id} value={r.id}>
                          <span className="text-[10px] text-muted-foreground mr-1">
                            {r.kind === "team" ? "GANG" : "SHOOTER"}
                          </span>
                          {r.name}
                        </SelectItem>
                      ))}
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  {pLogo ? (
                    ...
                    <div className="h-9 w-9 rounded bg-primary/15 grid place-items-center text-primary">
              ...
              <Button onClick={addParticipant} disabled={pLogoBusy}>
            ...
            <p className="text-[11px] text-muted-foreground">
              The order below is the bracket placement — use the arrows to decide who faces who. Pairs are formed
              top-to-bottom (1 vs 2, 3 vs 4, etc).
              ...
              {filteredParticipants.length > 0 ? (
                filteredParticipants.map((p, idx) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="flex items-center gap-2 rounded-md border border-primary/20 bg-card/60 px-2 py-1"
                    >
                      ...
                      {p.logo_url ? (
                        <img
                          src={p.logo_url}
                          alt=""
                          className="h-7 w-7 rounded object-cover border border-primary/30"
                        />
                      ) : (
                        <div className="h-7 w-7 rounded bg-primary/15 grid place-items-center text-[10px] font-bold text-primary">
                          {p.name.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <span className="min-w-0 flex-1 truncate text-sm font-semibold">{p.name}</span>
                      <button
                        onClick={() => moveParticipant(originalIndex, -1)}
                      ...
                      <button
                        onClick={() => moveParticipant(originalIndex, 1)}
                        disabled={originalIndex === participants.length - 1}
                        className="text-muted-foreground disabled:opacity-30 hover:text-primary"
                      >
                ...
                <span className="text-xs text-muted-foreground text-center py-4">
                  No shooters found matching "{searchQuery}"
                </span>
              ) : (
            ...
            <div className="flex flex-wrap gap-2 items-center pt-1">
              <Button className="btn-luxury" onClick={generateBracketPreview}>
                <Wand2 className="h-4 w-4 mr-1" />
                Preview & Customize Bracket
              </Button>
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  value={sel.futures_match_id ?? ""}
                  onChange={(e) => linkFutures(e.target.value)}
                >
      ...
      {sel && matches.length > 0 && (
        <Card className="glass p-2">
          <div className="text-xs text-muted-foreground px-2 py-1">
            Tap any matchup to enter scores and mark the winner. The winner advances automatically.
          </div>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="glass-strong border-primary/30 max-w-2xl backdrop-blur-2xl shadow-luxury overflow-hidden max-h-96 overflow-y-auto">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Assign participants to bracket slots. Click any empty slot to change the assignment.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-3">
            <p className="text-[11px] text-amber-200">
              <strong>Note:</strong> Your {participants.length} participants will fill {bracketSlots.length} bracket
              slots (rounded to nearest power of 2).
            </p>
              ...
              {bracketSlots.map((_, slotIdx) => {
                ...
                return (
                  <div key={slotIdx} className="border border-primary/30 rounded-lg p-2 bg-card/50">
                    ...
                    <Select
                      value={assigned ?? ""}
                      onValueChange={(val) => assignParticipantToSlot(slotIdx, val || null)}
                    >
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button className="btn-luxury" onClick={generateBracketFromPreview}>
              Generate Bracket
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="glass-strong border-primary/30 max-w-md backdrop-blur-2xl shadow-luxury overflow-hidden">
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {resultMatch?.label ?? "Match"} Result
            </DialogTitle>
            <DialogDescription>
              Link a live match to auto-fill scores, then mark each shooter Won, Qualified, Lost or Disqualified.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-1">
            ...
            {linkId && (
              <p className="text-[11px] text-emerald-400">
                Scores below are pulled from the live match. They auto-update whenever the match score changes.
              </p>
            )}
          </div>
          ...
          <DialogFooter className="flex-col gap-3 sm:flex-col">
            ...
            <div className="w-full space-y-1.5">
              <div className="text-xs font-bold flex items-center gap-1 text-amber-200 truncate">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                <Button
                  ...
                  disabled={!rA}
                  onClick={() => submitResult(resultMatch!.participant_a_id, "won")}
                >
                  Won
                </Button>
                <Button
                  ...
                  className="text-xs"
                  disabled={!rA}
                  onClick={() => submitResult(resultMatch!.participant_a_id, "qualified")}
                >
                  Qualified
                </Button>
                <Button
                  ...
                  className="text-xs"
                  disabled={!rB}
                  onClick={() => submitResult(resultMatch!.participant_b_id, "won")}
                >
                  Lost
                </Button>
                <Button
                  ...
                  className="text-xs"
                  disabled={!rB || !rA}
                  onClick={() => submitResult(resultMatch!.participant_b_id, "won", resultMatch!.participant_a_id)}
                >
                  Disqualify
                </Button>
              </div>
            ...
            <div className="w-full space-y-1.5">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                ...
                <Button
                  ...
                  className="text-xs"
                  disabled={!rB}
                  onClick={() => submitResult(resultMatch!.participant_b_id, "qualified")}
                >
                  Qualified
                </Button>
                <Button
                ...
                <Button
                  ...
                  className="text-xs"
                  disabled={!rA || !rB}
                  onClick={() => submitResult(resultMatch!.participant_a_id, "won", resultMatch!.participant_b_id)}
                >
                  Disqualify
                </Button>
              </div>
            </div>
            <Button variant="outline" className="w-full" onClick={() => submitResult(null)}>
              Save scores only (no winner yet)
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
